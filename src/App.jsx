  ...
  try {
    ...
    if (ordersRows) {
      ordersRows.forEach(o => {
        // Çağrı kayıtlarını tableCalls'a al
        if (o.status && o.status.startsWith("cagri_")) {
          const callType = o.status.replace("cagri_", "");
          const existing = s.tableCalls[o.table_id];
          // En son çağrıyı al
          if (!existing || (o.timestamp || 0) > (existing.timestamp || 0)) {
            s.tableCalls[o.table_id] = {
              type: callType, time: o.time,
              timestamp: o.timestamp, read: false, orderId: o.id
            };
          }
          return;
        }
...
export default function App() {
  ...
  const [page, setPage] = useState(urlMutfak ? "mutfak" : urlMasa ? "menu" : "home");
...
// HOME PAGE
// ═══════════════════════════════════════════════════════════════════════════════
function HomePage({ onMenu, onAdmin, onRezervasyon, onMenuView, onMutfak }) {
  const store = gs();
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0700", fontFamily: "'Lato',Georgia,sans-serif", position: "relative" }}>
      ...
      <div style={{ background: "linear-gradient(180deg,#1c0e00,#0f0700)", padding: "20px 16px 0", position: "sticky", top: 0, zIndex: 50 }}>
        {onBack && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <button onClick={onBack} style={{ background: "rgba(255,255,255,0.08)", border: "none", color: "#e8a020", borderRadius: 8, padding: "5px 11px", cursor: "pointer", fontSize: 13 }}>←</button>
          {!viewOnly && (
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => {
                const s = gs();
                s.tableCalls = s.tableCalls || {};
                s.tableCalls[tableId] = { type: "garson", time: new Date().toLocaleTimeString("tr-TR"), read: false };
                ss(s); refresh();
                pushCallToSb(tableId, s.tableCalls[tableId]);
                alert("Garson çağrıldı! Kısa süre içinde gelecek.");
              }} style={{ background: "rgba(232,160,32,0.15)", border: "1px solid rgba(232,160,32,0.3)", color: "#e8a020", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                🙋 Garson Çağır
              </button>
              <button onClick={() => {
                const s = gs();
                s.tableCalls = s.tableCalls || {};
                s.tableCalls[tableId] = { type: "hesap", time: new Date().toLocaleTimeString("tr-TR"), read: false };
                ss(s); refresh();
                pushCallToSb(tableId, s.tableCalls[tableId]);
                alert("Hesap talebi iletildi!");
              }} style={{ background: "rgba(184,58,12,0.2)", border: "1px solid rgba(184,58,12,0.4)", color: "#f0a080", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                🧾 Hesap İste
              </button>
            </div>
          )}
...
function useCallNotifications(store) {
  ...
  useEffect(() => {
    const calls = store.tableCalls || {};
    const newCalls = [];
    Object.entries(calls).forEach(([tid, call]) => {
      // timestamp + type ile unique key
      const key = tid + "_" + (call.timestamp || call.time) + "_" + call.type;
      if (!call.read && !knownCallKeys.current.has(key)) {
        knownCallKeys.current.add(key);
        newCalls.push({ tableId: tid, type: call.type, time: call.time });
      }
    });
    if (newCalls.length > 0) {
      playCallBeep();
      if (toastTimer.current) clearTimeout(toastTimer.current);
      // En son çağrıyı göster
  ...
  // store.tableCalls değişimini JSON stringify ile takip et
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f0ea", fontFamily: "'Lato',Georgia,sans-serif" }}>
      ...
      <div style={{ display: "flex", background: "#fff", borderBottom: "2px solid #e8e0d5", overflowX: "auto" }}>
        {[
          { id: "map", label: `🗺️ Masa Haritası${Object.values(store.tableCalls||{}).filter(c=>!c.read).length ? ` 🔔${Object.values(store.tableCalls||{}).filter(c=>!c.read).length}` : ""}` },
          ...
          <div style={{ background: callToast.type === "hesap" ? "#7f1d1d" : "#1c0e00", color: "#fff", borderRadius: 14, padding: "14px 18px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.4)", border: callToast.type === "hesap" ? "1.5px solid #ef4444" : "1.5px solid #e8a020" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={async () => {
                const s = gs();
                if (s.tableCalls && s.tableCalls[callToast.tableId]) {
                  const orderId = s.tableCalls[callToast.tableId].orderId;
                  s.tableCalls[callToast.tableId].read = true;
                  ss(s); refresh();
                  // Supabase'den çağrı kaydını sil
                  if (orderId) await sbDelete("orders", orderId);
                }
                dismissCallToast();
          ...
          <div key={tid} id={`table-card-${tid}`} style={{ marginBottom: 24, background: "#fff", borderRadius: 18, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", border: "1px solid #f0e8de" }}>
            ...
            <div style={{ background: "linear-gradient(90deg,#1c0e00,#2d1500)", padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {/* Çağrı göstergesi */}
                {store.tableCalls && store.tableCalls[tid] && !store.tableCalls[tid].read && (
                  <span style={{ background: store.tableCalls[tid].type === "hesap" ? "#ef4444" : "#e8a020", color: "#fff", borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 700, animation: "pulse 1.5s infinite", cursor: "pointer" }}
                    onClick={async () => {
                      const s = gs();
                      const orderId = s.tableCalls[tid]?.orderId;
                      if (s.tableCalls[tid]) s.tableCalls[tid].read = true;
                      ss(s); refresh();
                      if (orderId) await sbDelete("orders", orderId);
                    }}>
                    {store.tableCalls[tid].type === "hesap" ? "🧾 Hesap!" : "🙋 Garson!"}
                  </span>
                )}
...
function KitchenPage({ onBack }) {
  ...
  // Saat güncelle
  useEffect(() => {
    const iv = setInterval(() => setClock(new Date().toLocaleTimeString("tr-TR")), 1000);
    return () => clearInterval(iv);
  }, []);
  ...
  useEffect(() => {
    if (!auth) return;
    const allOrders = Object.values(store.activeOrders).flat().filter(o => o.status === "beklemede");
  ...
  if (!auth) return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,#0f0700,#1c0e00)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'Lato',sans-serif" }}>
      ...
      <div style={{ width: "100%", maxWidth: 320, textAlign: "center" }}>
        ...
        <h2 style={{ color: "#e8a020", fontSize: 22, margin: "0 0 6px", fontFamily: "'Playfair Display',serif" }}>Garson Ekranı</h2>
        <p style={{ color: "#a07850", fontSize: 13, marginBottom: 24 }}>Siparişler & Rezervasyonlar</p>
  ...
  // ── Veri hazırla ──
  const activeOrders = Object.entries(store.activeOrders)
    .flatMap(([tid, orders]) => orders
      .filter(o => o.status !== "teslim edildi" && o.status !== "tamamlandı")
      .map(o => ({ ...o, tid })))
    .sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));

  const bekleyenCount = activeOrders.filter(o => o.status === "beklemede").length;
  const today = new Date().toISOString().slice(0, 10);
  const reservations = (store.reservations || [])
    .filter(r => r.date >= today && r.status !== "iptal")
    .sort((a, b) => a.date.localeCompare(b.date) || a.time.localeCompare(b.time));
  const bekleyenResCount = reservations.filter(r => r.status === "beklemede").length;

  const statusInfo = (s) => ({
    beklemede:     { bg: "#2a0a0a", border: "#ef4444", dot: "#ef4444", label: "Bekliyor" },
    "hazırlanıyor":{ bg: "#1a1200", border: "#f59e0b", dot: "#f59e0b", label: "Hazırlanıyor" },
    hazır:         { bg: "#0a1a0a", border: "#22c55e", dot: "#22c55e", label: "Hazır" },
  })[s] || { bg: "#1a1a1a", border: "#555", dot: "#555", label: s };

  const resStatusInfo = (s) => ({
    beklemede: { bg: "#fef9c3", color: "#713f12", label: "⏳ Bekliyor" },
    "onaylı":  { bg: "#dcfce7", color: "#166534", label: "✓ Onaylı" },
  })[s] || { bg: "#fee2e2", color: "#991b1b", label: s };

  // ── Ana ekran ──
  return (
    <div style={{ minHeight: "100vh", background: "#111", fontFamily: "'Lato',sans-serif", color: "#e0e0e0" }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ background: "#1c0e00", borderBottom: "2px solid #b83a0c", padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 24 }}>🧾</span>
          <div>
            <div style={{ color: "#e8a020", fontWeight: 700, fontSize: 16, fontFamily: "'Playfair Display',serif" }}>{gs().restaurant?.name || "Garson Ekranı"}</div>
            <div style={{ color: "#7a5535", fontSize: 11 }}>{clock}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {bekleyenCount > 0 && (
            <span style={{ background: "#ef4444", color: "#fff", borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 700, animation: "pulse 1.5s infinite" }}>
              {bekleyenCount} sipariş
            </span>
          )}
          {bekleyenResCount > 0 && (
            <span style={{ background: "#f59e0b", color: "#000", borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>
              {bekleyenResCount} rezervasyon
            </span>
          )}
      ...
      <div style={{ display: "flex", background: "#1a1a1a", borderBottom: "2px solid #333" }}>
        {[
          { id: "siparisler", label: `🍽️ Siparişler${bekleyenCount ? ` 🔴${bekleyenCount}` : ""}` },
          { id: "rezervasyonlar", label: `📅 Rezervasyonlar${bekleyenResCount ? ` 🟡${bekleyenResCount}` : ""}` },
        ].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ flex: 1, padding: "14px 10px", border: "none", borderBottom: tab === t.id ? "3px solid #e8a020" : "3px solid transparent", background: "none", color: tab === t.id ? "#e8a020" : "#666", fontWeight: tab === t.id ? 700 : 400, fontSize: 14, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: 16, maxWidth: 800, margin: "0 auto" }}>
        ...
        {tab === "siparisler" && (
          activeOrders.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "#444" }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>✅</div>
              <div style={{ fontSize: 16, color: "#4ade80" }}>Bekleyen sipariş yok</div>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {activeOrders.map(order => {
                ...
                return (
                  <div key={order.id} style={{ background: st.bg, border: `2px solid ${st.border}`, borderRadius: 14, overflow: "hidden" }}>
                    <div style={{ padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${st.border}44` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ width: 10, height: 10, borderRadius: "50%", background: st.dot, display: "inline-block" }} />
                        <span style={{ fontWeight: 700, color: "#e0e0e0" }}>Masa {order.tid}</span>
                        <span style={{ background: `${st.border}33`, color: st.dot, borderRadius: 20, padding: "2px 10px", fontSize: 11, fontWeight: 700 }}>{st.label}</span>
                      </div>
                      <span style={{ color: "#666", fontSize: 12 }}>{elapsed > 0 ? `${elapsed} dk` : "Az önce"}</span>
                    </div>
                    <div style={{ padding: "10px 16px" }}>
                      {(order.items || []).map((item, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: 14 }}>
                          <span style={{ color: "#e0e0e0", fontWeight: 600 }}>{item.name}</span>
                          <span style={{ color: "#e8a020", fontWeight: 700 }}>×{item.qty}</span>
                        </div>
                      ))}
                      {order.note && (
                        <div style={{ marginTop: 8, background: "rgba(232,160,32,0.1)", borderRadius: 8, padding: "6px 10px", fontSize: 12, color: "#e8a020" }}>📝 {order.note}</div>
                      )}
                    </div>
                    <div style={{ padding: "10px 16px", display: "flex", gap: 8 }}>
                      {order.status === "beklemede" && (
                        <button onClick={() => setOrderStatus(order.tid, order.id, "hazırlanıyor")}
                          style={{ flex: 1, background: "#f59e0b", color: "#000", border: "none", borderRadius: 8, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                          🍳 Hazırlanıyor
                        </button>
                      )}
                      {(order.status === "beklemede" || order.status === "hazırlanıyor") && (
                        <button onClick={() => setOrderStatus(order.tid, order.id, "hazır")}
                          style={{ flex: 1, background: "#16a34a", color: "#fff", border: "none", borderRadius: 8, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                          ✓ Hazır
                        </button>
                      )}
                      {order.status === "hazır" && (
                        <button onClick={() => setOrderStatus(order.tid, order.id, "teslim edildi")}
                          style={{ flex: 1, background: "#2563eb", color: "#fff", border: "none", borderRadius: 8, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                          🚀 Teslim Edildi
                        </button>
                      )}
                    </div>
        ...
        {tab === "rezervasyonlar" && (
          reservations.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "#444" }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>📅</div>
              <div style={{ fontSize: 16, color: "#888" }}>Yaklaşan rezervasyon yok</div>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {reservations.map(r => {
                const rs = resStatusInfo(r.status);
                return (
                  <div key={r.id} style={{ background: "#1a1a1a", border: "1px solid #333", borderRadius: 14, padding: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 15, color: "#e0e0e0" }}>{r.name}</div>
                    ...
                    {r.status === "beklemede" && (
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => approveRes(r.id)}
                          style={{ flex: 1, background: "#16a34a", color: "#fff", border: "none", borderRadius: 8, padding: "9px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                          ✓ Onayla
                        </button>
                        <button onClick={() => cancelRes(r.id)}
                    ...
                    {r.status === "onaylı" && (
                      <button onClick={() => cancelRes(r.id)}
                        style={{ background: "#7f1d1d", color: "#fca5a5", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, cursor: "pointer" }}>
                        İptal Et
                      </button>
                    )}
                  </div>

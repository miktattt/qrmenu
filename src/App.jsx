...
function KitchenPage({ onBack }) {
  ...
  const activeCalls = Object.entries(store.tableCalls || {})
    .filter(([, c]) => !c.read)
    ...
    <div style={{ minHeight: "100vh", background: "#111", fontFamily: "'Lato',sans-serif", color: "#e0e0e0" }}>
      ...
      <div style={{ background: "#1c0e00", borderBottom: "2px solid #b83a0c", padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {callCount > 0 && (
            <span style={{ background: "#e8a020", color: "#1c0e00", borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 700, animation: "pulse 1.5s infinite" }}>
              🔔 {callCount} çağrı
            </span>
          )}
          {bekleyenResCount > 0 && (
      ...
      <div style={{ display: "flex", background: "#1a1a1a", borderBottom: "2px solid #333" }}>
        {[
          ...
          { id: "cagrilar", label: `🔔 Çağrılar${callCount ? ` 🟡${callCount}` : ""}` },
      ...
      <div style={{ padding: 16, maxWidth: 800, margin: "0 auto" }}>
        ...
        {tab === "cagrilar" && (
          activeCalls.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "#444" }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>🔔</div>
              <div style={{ fontSize: 16, color: "#888" }}>Bekleyen çağrı yok</div>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {activeCalls.map(([tid, call]) => (
                <div key={tid + call.timestamp} style={{
                  background: call.type === "hesap" ? "#1a0505" : "#1a1200",
                  border: `2px solid ${call.type === "hesap" ? "#ef4444" : "#e8a020"}`,
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 18, color: call.type === "hesap" ? "#f87171" : "#e8a020" }}>
                        {call.type === "hesap" ? "Hesap İsteniyor" : "Garson Çağrısı"}
                      </div>
                      <div style={{ color: "#888", fontSize: 13, marginTop: 3 }}>
                        Masa <strong style={{ color: "#e0e0e0" }}>{tid}</strong> · {call.time}
                      </div>
                    </div>
                  </div>
                  <button onClick={async () => {
                    ...
                    if (s.tableCalls?.[tid]) {
                      const orderId = s.tableCalls[tid].orderId;
                      s.tableCalls[tid].read = true;
                      ss(s); refresh();
                      if (orderId) await sbDelete("orders", orderId);
                    }
                  }} style={{ background: "#16a34a", color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 14, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                    ✓ Görüldü
                  </button>
                </div>
              ))}

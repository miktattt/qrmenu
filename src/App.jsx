...
function KitchenPage({ onBack }) {
  ...
  // Sadece beklemede + hazırlanıyor siparişler, zamana göre sıralı
  const orders = Object.entries(store.activeOrders)
    .flatMap(([tid, os]) => os
      .filter(o => o.status === "beklemede" || o.status === "hazırlanıyor")
      .map(o => ({ ...o, tid })))
    .sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));

  const bekleyenCount = orders.filter(o => o.status === "beklemede").length;
    ...
    <div style={{ minHeight: "100vh", background: "#0d0d0d", fontFamily: "'Lato',sans-serif" }}>
      ...
      <div style={{ background: "#0a1200", borderBottom: "3px solid #16a34a", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {bekleyenCount > 0 && (
            <span style={{ background: "#ef4444", color: "#fff", borderRadius: 20, padding: "5px 14px", fontSize: 14, fontWeight: 700, animation: "blink 1s infinite" }}>
              🔴 {bekleyenCount} yeni sipariş
            </span>
          )}
      ...
      <div style={{ padding: 16, maxWidth: 700, margin: "0 auto" }}>
        {orders.length === 0 ? (
          ...
          const isReady = order.status === "hazırlanıyor";
          const elapsed = order.timestamp ? Math.floor((Date.now() - order.timestamp) / 60000) : 0;
          return (
            <div key={order.id} style={{
              background: isReady ? "#0a1a0a" : "#1a0a0a",
              border: `3px solid ${isReady ? "#16a34a" : "#ef4444"}`,
              borderRadius: 18, marginBottom: 20, overflow: "hidden"
            }}>
              {/* Masa başlığı — büyük, belirgin */}
              <div style={{ background: isReady ? "#16a34a" : "#ef4444", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "#fff", fontWeight: 900, fontSize: 28, fontFamily: "'Playfair Display',serif", letterSpacing: -0.5 }}>
                  MASA {order.tid}
                </span>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: "rgba(255,255,255,0.9)", fontSize: 13, fontWeight: 700 }}>{isReady ? "🍳 HAZIRLANIYOR" : "🔴 YENİ SİPARİŞ"}</div>
              ...
              <div style={{ padding: "16px 20px" }}>
                {(order.items || []).map((item, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <span style={{ color: isReady ? "#4ade80" : "#ef4444", fontWeight: 900, fontSize: 24, minWidth: 44, textAlign: "right" }}>×{item.qty}</span>
                  </div>
                ))}
                {order.note && (
              ...
              <div style={{ padding: "0 20px 20px" }}>
                {!isReady ? (
                  <button onClick={() => setOrderStatus(order.tid, order.id, "hazırlanıyor")}
                    style={{ width: "100%", background: "#ca8a04", color: "#fff", border: "none", borderRadius: 12, padding: "18px", fontSize: 18, fontWeight: 900, cursor: "pointer", letterSpacing: 0.5 }}>
                    🍳 HAZIRLANIYOR
                  </button>
                ) : (
                  <button onClick={() => setOrderStatus(order.tid, order.id, "hazır")}
                    style={{ width: "100%", background: "#16a34a", color: "#fff", border: "none", borderRadius: 12, padding: "18px", fontSize: 18, fontWeight: 900, cursor: "pointer", letterSpacing: 0.5 }}>
                    ✅ HAZIR
                  </button>
                )}
              </div>
...
function WaiterPage({ onBack }) {
  ...
  useEffect(() => {
    ...
    if (newReady.length > 0) {
      newReady.forEach(o => knownReadyIds.current.add(o.id));
      try {
        const ctx = new (window.AudioContext || window.webkitAudioContext)();
        [0, 0.2, 0.4].forEach(t => {
          const osc = ctx.createOscillator(); const g = ctx.createGain();
  ...
  }, [JSON.stringify(Object.values(store.activeOrders).flat().map(o => o.id + o.status))]);
  ...
  const statusColor = (s) => ({ beklemede: "#f59e0b", "hazırlanıyor": "#3b82f6", hazır: "#22c55e", "teslim edildi": "#9ca3af" })[s] || "#9ca3af";
    ...
    <div style={{ minHeight: "100vh", background: "#f8f5f0", fontFamily: "'Lato',sans-serif" }}>
      ...
      <div style={{ padding: 14, maxWidth: 700, margin: "0 auto" }}>
        ...
        {tab === "siparisler" && (
          activeTables.length === 0 ? (
            ...
            return (
              <div key={tid} style={{ background: "#fff", borderRadius: 16, marginBottom: 16, overflow: "hidden", boxShadow: hasReady ? "0 0 0 3px #22c55e" : "0 2px 12px rgba(0,0,0,0.08)", border: "1px solid #f0e8de", transition: "box-shadow .3s" }}>
                {/* Masa header */}
                ...
                {pending.map(order => (
                  <div key={order.id} style={{ borderBottom: "1px solid #f5ede5", padding: "10px 16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ width: 9, height: 9, borderRadius: "50%", background: statusColor(order.status), display: "inline-block", transition: "background .5s" }} />
                        <span style={{ fontSize: 12, fontWeight: 700, color: statusColor(order.status), transition: "color .5s" }}>{statusLabel(order.status)}</span>
                        <span style={{ color: "#bbb", fontSize: 11 }}>{order.time}</span>
                      </div>
                      <button onClick={() => setEditOrder({ order: { ...order, items: [...order.items] }, tid })}
                        style={{ background: "#f5f0ea", border: "none", borderRadius: 6, padding: "3px 8px", fontSize: 12, cursor: "pointer", color: "#888" }}>✏️</button>
                    </div>
                    {order.items.map((item, i) => (
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                      {order.status === "hazırlanıyor" && (
                        <button onClick={() => setOrderStatus(tid, order.id, "beklemede")}
                          style={{ background: "#fef3c7", color: "#92400e", border: "none", borderRadius: 6, padding: "5px 10px", fontSize: 11, cursor: "pointer" }}>↩ Geri Al</button>
                      )}
                      {order.status === "hazır" && (
                        <button onClick={() => setOrderStatus(tid, order.id, "teslim edildi")}
                          style={{ flex: 1, background: "#2563eb", color: "#fff", border: "none", borderRadius: 8, padding: "8px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>🚀 Teslim Edildi</button>
                      )}
                    </div>
                  </div>
                ))}
      ...
      {editOrder && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 400, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setEditOrder(null)}>
          <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", padding: 20, width: "100%", maxWidth: 500, maxHeight: "80vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            {editOrder.order.items.map((item, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #f5ede5" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <button onClick={() => setEditOrder(p => { const items=[...p.order.items]; if(items[i].qty>1)items[i]={...items[i],qty:items[i].qty-1};else items.splice(i,1); return{...p,order:{...p.order,items}}; })} style={{ background: "#fee2e2", color: "#b91c1c", border: "none", borderRadius: 6, width: 30, height: 30, fontSize: 16, cursor: "pointer" }}>−</button>
                  <span style={{ fontWeight: 700, minWidth: 24, textAlign: "center", color: "#b83a0c" }}>{item.qty}</span>
                  <button onClick={() => setEditOrder(p => { const items=[...p.order.items]; items[i]={...items[i],qty:items[i].qty+1}; return{...p,order:{...p.order,items}}; })} style={{ background: "#dcfce7", color: "#166534", border: "none", borderRadius: 6, width: 30, height: 30, fontSize: 16, cursor: "pointer" }}>+</button>
                </div>
                <span style={{ color: "#b83a0c", minWidth: 55, textAlign: "right", fontSize: 13, fontWeight: 600 }}>{fmt(item.price*item.qty)}</span>
              </div>
            ))}
            <textarea value={editOrder.order.note||""} onChange={e => setEditOrder(p=>({...p,order:{...p.order,note:e.target.value}}))} rows={2} placeholder="Not..."
              style={{ width:"100%", marginTop:12, border:"1px solid #f0e8de", borderRadius:8, padding:"8px 10px", fontSize:13, fontFamily:"inherit", resize:"none", boxSizing:"border-box", outline:"none" }} />
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:12, paddingTop:10, borderTop:"1px solid #f0e8de" }}>
              <span style={{ color:"#888" }}>Toplam</span>
              <span style={{ fontWeight:700, color:"#b83a0c", fontSize:17 }}>{fmt(editOrder.order.items.reduce((s,i)=>s+i.price*i.qty,0))}</span>
            </div>
            <button onClick={async () => {
              const s = gs();
              ...
              setEditOrder(null);
            }} style={{ width:"100%", marginTop:12, background:"#16a34a", color:"#fff", border:"none", borderRadius:10, padding:"13px", fontSize:15, fontWeight:700, cursor:"pointer" }}>
              💾 Kaydet
            </button>
          </div>
        </div>

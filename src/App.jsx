...
async function pushCallToSb(tableId, callData) {
  ...
  try {
    if (!callData) {
      await sb.from("orders").update({ call_data: null }).eq("table_id", tableId).in("status", ["beklemede","hazırlanıyor","hazır"]);
    } else {
      // orders tablosundaki en güncel siparişe call_data yaz — ayrı tablo açmadan
      const store = gs();
      const orders = store.activeOrders[tableId] || [];
      if (orders.length > 0) {
        await sb.from("orders").update({ call_data: callData }).eq("id", orders[orders.length - 1].id);
      }
    }
    return true;
  } catch(e) { return false; }
...
function useCallNotifications(store) {
  ...
  useEffect(() => {
    ...
    Object.entries(calls).forEach(([tid, call]) => {
      const key = tid + "_" + call.time;
      if (!call.read && !knownCallKeys.current.has(key)) {
        knownCallKeys.current.add(key);
        playCallBeep();
        if (toastTimer.current) clearTimeout(toastTimer.current);
        setCallToast({ tableId: tid, type: call.type, time: call.time, uid: Date.now() });
        toastTimer.current = setTimeout(() => setCallToast(null), 8000);
      }
    });
  }, [store.tableCalls]);
        ...
        {callToast && (
          <div style={{ background: callToast.type === "hesap" ? "#7f1d1d" : "#1c0e00", color: "#fff", borderRadius: 14, padding: "14px 18px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.4)", border: callToast.type === "hesap" ? "1.5px solid #ef4444" : "1.5px solid #e8a020" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => {
                const s = gs();
                if (s.tableCalls && s.tableCalls[callToast.tableId]) s.tableCalls[callToast.tableId].read = true;
                ss(s); refresh(); dismissCallToast();
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {store.tableCalls && store.tableCalls[tid] && !store.tableCalls[tid].read && (
                  <span style={{ background: store.tableCalls[tid].type === "hesap" ? "#ef4444" : "#e8a020", color: "#fff", borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 700, animation: "pulse 1.5s infinite", cursor: "pointer" }}
                    onClick={() => { const s = gs(); s.tableCalls[tid].read = true; ss(s); refresh(); }}>

    ...
    <div style={{ minHeight: "100vh", background: "#0d0d0d", fontFamily: "'Lato',sans-serif" }}>
      ...
      <div style={{ padding: 16, maxWidth: 700, margin: "0 auto" }}>
        {orders.length === 0 ? (
          ...
          const isNew = order.status === "beklemede";
          const borderColor = isPreparing ? "#ca8a04" : "#ef4444";
          const bgColor = isPreparing ? "#1a1200" : "#1a0505";
          const headerBg = isPreparing ? "#ca8a04" : "#ef4444";
              ...
              <div style={{ padding: "16px 20px" }}>
                {(order.items || []).map((item, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <span style={{ color: isPreparing ? "#fbbf24" : "#f87171", fontWeight: 900, fontSize: 24, minWidth: 44, textAlign: "right" }}>×{item.qty}</span>
                  </div>
                ))}
                {order.note && (
              ...
              <div style={{ padding: "0 20px 20px" }}>
                {isNew && (
                  <button onClick={() => setOrderStatus(order.tid, order.id, "hazırlanıyor")}
                    style={{ width: "100%", background: "#ca8a04", color: "#fff", border: "none", borderRadius: 12, padding: "18px", fontSize: 18, fontWeight: 900, cursor: "pointer" }}>
                    🍳 HAZIRLANIYOR
                  </button>
                )}
                {isPreparing && (
                ...
                {pending.map(order => (
                  <div key={order.id} style={{ borderBottom: "1px solid #f5ede5", padding: "10px 16px" }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
                      {/* Beklemede: Hazırlanıyor butonu */}
                      {order.status === "beklemede" && (
                        <button onClick={() => setOrderStatus(tid, order.id, "hazırlanıyor")}
                          style={{ flex: 1, background: "#ca8a04", color: "#fff", border: "none", borderRadius: 8, padding: "8px 12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                          🍳 Hazırlanıyor
                        </button>
                      )}
                      {/* Hazırlanıyor: Hazır butonu + Geri Al */}
                      {order.status === "hazırlanıyor" && (
                        <>
                          <button onClick={() => setOrderStatus(tid, order.id, "beklemede")}
                            style={{ background: "#fef3c7", color: "#92400e", border: "1px solid #fbbf24", borderRadius: 8, padding: "8px 10px", fontSize: 12, cursor: "pointer" }}>↩</button>
                          <button onClick={() => setOrderStatus(tid, order.id, "hazır")}
                            style={{ flex: 1, background: "#16a34a", color: "#fff", border: "none", borderRadius: 8, padding: "8px 12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                            ✅ Hazır
                          </button>
                        </>
                      )}
                      {/* Hazır: Teslim Et butonu */}
                      {order.status === "hazır" && (
                        <button onClick={() => setOrderStatus(tid, order.id, "teslim edildi")}
                          style={{ flex: 1, background: "#2563eb", color: "#fff", border: "none", borderRadius: 8, padding: "8px 12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                          🚀 Teslim Edildi
                        </button>
                      )}
                    </div>
